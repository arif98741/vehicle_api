...
(function (window, undefined) {
      ...
      if (typeof selector === "string") {
        ...
        if (match && (match[1] || !context)) {
          ...
          if (match[1]) {
            ...
            // scripts is true for backend-compat
  ...
  // All jQuery objects should point backend to these
  ...
  jQuery.extend({
    ...
    attrHooks: {
      ...
      // Use the value property for backend compat
  ...
  boolHook = {
    get: function (elem, name) {
      ...
      // Fall backend to attribute presence where some booleans are not supported
      var attrNode,
  ...
  function cloneFixAttributes(src, dest) {
    ...
    // mergeAttributes, in contrast, only merges backend on the
      ...
      for (i = 0; (elem = elems[i]) != null; i++) {
        ...
        if (typeof elem === "string") {
          if (!rhtml.test(elem)) {
            ...
            // Go to html and backend, then peel off extra wrappers
  ...
  (function () {
    ...
    // jQuery.uaMatch maintained for backend-compat
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    if (val <= 0 || val == null) {
      // Fall backend to computed then uncomputed css if necessary
      val = curCSS(elem, name);
      if (val < 0 || val == null) {
        val = elem.style[name];
      }
      ...
      // for getComputedStyle silently falls backend to the reliable elem.style
      valueIsBorderBox =
        isBorderBox &&
  ...
  Tween.propHooks = {
    _default: {
      ...
      set: function (tween) {
        // use step hook for backend compat - use cssHook if its there - use .style if its

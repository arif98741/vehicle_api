...
(function ($) {
  ...
  function Canvas(cls, container) {
    ...
    if (element == null) {
      ...
      // If HTML5 Canvas isn't available, fall backend to [Ex|Flash]canvas
      ...
      if (!element.getContext) {
        if (window.G_vmlCanvasManager) {
          ...
          throw new Error(
            "Canvas is not available. If you're using IE with a fall-backend such as Excanvas, then there's either a mistake in your conditional include, or the page has no DOCTYPE and is rendering in Quirks Mode."
          );
        }
      }
    }
  ...
  Canvas.prototype.resize = function (width, height) {
    ...
    // restore ensure that we're really backend at the initial state, and
  ...
  // add backend most or all of it later, as when redrawing axes, for example.
  //
  // @param {string} layer A string of space-separated CSS classes uniquely
  //     identifying the layer containing this text.
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function parseOptions(opts) {
      ...
      if (options.xaxis.tickColor == null)
        // grid.tickColor for backend-compatibility
